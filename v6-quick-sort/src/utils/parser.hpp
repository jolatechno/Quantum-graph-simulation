...
std::tuple<state_t,
    ...
    size_t, int, size_t> iteration_parser(
    ...
    options.add_options() ("h,help", "Print help")
        ...
        ("n-graph", "number of random graphs to start with", cxxopts::value<size_t>()->default_value("1"))

        ("n-fast", "number of fast iteration in-between normal iterations", cxxopts::value<size_t>()->default_value("0"))
        ("start-serializing", "iteration for which to start serializing", cxxopts::value<size_t>()->default_value("0"))

        ("n,n-iter", "number of iteration", cxxopts::value<size_t>()->default_value("3"))
        ...
        ("P,precision", "number of bits of precision", cxxopts::value<size_t>()->default_value("128"))
        ...
        ("niter-1", "number of application of the first rule per iteration", cxxopts::value<size_t>()->default_value("1"))
        ("niter-2", "number of application of the second rule per iteration", cxxopts::value<size_t>()->default_value("1"))
        ...
        ("s,size", "starting size", cxxopts::value<size_t>()->default_value("8"))
    ...
    SET_PRECISION(result["precision"].as<size_t>())
    safety_margin = result["safety-margin"].as<float>();

    // ------------------------------------------
    // parameters

    size_t n_iter = result["n-iter"].as<size_t>();
    ...
    size_t num_graph = result["n-graph"].as<size_t>();
    ...
    rule->n_iter = rule->move ? result["niter-1"].as<size_t>() : 1;
    ...
    if (result.count("rule2")) {
        ...
        rule2->n_iter = rule2->move ? result["niter-2"].as<size_t>() : 1;
    }

    return {state,
        rule, rule2, 
        n_iter, result["start-serializing"].as<size_t>(), result["n-fast"].as<size_t>()};
}

std::tuple<state_t,
    state_t::rule_t*, state_t::rule_t*, 
    size_t, int> probabilist_parser(
    cxxopts::Options &options, int argc, char* argv[]) {
    ...
    options.add_options() ("h,help", "Print help")
        ...
        ("N,n-graph", "number of random graphs to start with", cxxopts::value<size_t>()->default_value("3"))
        ("start-serializing", "iteration for which to start serializing", cxxopts::value<size_t>()->default_value("0"))
    ...
    return {state,
        rule, rule2, 
        n_iter, result["start-serializing"].as<size_t>()};
}

std::tuple<state_t, state_t::rule_t*, state_t::rule_t*, size_t, int, size_t> test_parser(
    cxxopts::Options &options, int argc, char* argv[]) {
    ...
    options.add_options() ("h,help", "Print help")
        ...
        ("n,n-iter", "number of iteration", cxxopts::value<size_t>()->default_value("3"))
        ("R,n-reversed-iter", "number of reversed iteration", cxxopts::value<size_t>()->default_value("0"))
    ...
    size_t n_iter = result["n-iter"].as<size_t>();
    size_t n_reversed_iteration = result["n-reversed-iter"].as<size_t>();
    ...
    if (rule_ == "split_merge") {
        ...
        throw;
    return {state, rule, reversed_rule, n_iter, n_reversed_iteration, result["n-fast"].as<size_t>()};
}