...
public:
	...
	virtual void populate_new_graph(state_t const &s, state_t &next_state, unsigned int next_gid, unsigned int parent_id, unsigned int child_id) const {} /* step (8) */
...
Iteration protocol is:
  ...
  - (3): read the total numer of child graph generated by each parent graph and populate "next_gid", "nex_t_state.parent_gid" and "nex_t_state.child_id"
  ...
  - (6): reserve all public vector for a new state by iterating over the N first "next_gid" andusing "nex_t_state.parent_gid" and "nex_t_state.child_id",
  	and assign "node_begin" and "sub_node_begin".

  - (7): generate all new graphs of the new state by iterating over the N first "next_gid" andusing "nex_t_state.parent_gid" and "nex_t_state.child_id".
...
public:
	...
	*/
	...
	/* step function */
	void step(state_t &nex_t_state, rule_t &rule) { step(nex_t_state, rule, -1); }
	void step(state_t &nex_t_state, rule_t &rule, unsigned int max_num_graphs) {
		...
		nex_t_state.resize_num_graphs_symbolic(total_num_graphs);
		...
		for (unsigned int gid = 0; gid < num_graphs; ++gid) {
			...
			for (unsigned int child_id_ = 0; child_id_ < num_child; ++child_id_, ++id) {
				nex_t_state.parent_gid[id] = gid;
				nex_t_state.child_id[id] = child_id_;
			}
		}
			...
			for (unsigned int gid = 0; gid < total_num_graphs; ++gid) {
				auto [hash_, real_, imag_, size_node_, size_sub_node_] = rule.child_properties(*this, nex_t_state.parent_gid[gid], nex_t_state.child_id[gid]);

				/* assign propreties for each child */
				nex_t_state.next_hash[gid] = hash_;
				nex_t_state.next_real[gid] = real_;
				nex_t_state.next_imag[gid] = imag_;
				nex_t_state.node_begin[gid] = size_node_;
				nex_t_state.sub_node_begin[gid] = size_sub_node_;
			}
		//}
		...
		__gnu_parallel::sort(nex_t_state.next_gid.begin(), nex_t_state.next_gid.begin() + total_num_graphs, [&](unsigned int const &gid1, unsigned int const &gid2) {
			return nex_t_state.next_hash[gid1] > nex_t_state.next_hash[gid2];
		});

		/* compute is_first_index */
		nex_t_state.is_first_index[nex_t_state.next_gid[0]] = true;

		#pragma omp parallel
		{
			#pragma omp for
			for (unsigned int gid = 1; gid < total_num_graphs; ++gid)
				nex_t_state.is_first_index[nex_t_state.next_gid[gid]] = nex_t_state.next_hash[nex_t_state.next_gid[gid]] != nex_t_state.next_hash[nex_t_state.next_gid[gid - 1]];
			...
			#pragma omp for
			for (unsigned int gid = 0; gid < total_num_graphs; ++gid) {
				auto id = nex_t_state.next_gid[gid];

				if (nex_t_state.is_first_index[id])
					/* sum magnitude of equal graphs */
					for (unsigned int gid_ = gid + 1; gid_ < total_num_graphs && !nex_t_state.is_first_index[nex_t_state.next_gid[gid_]]; ++gid_) {
						auto id_ = nex_t_state.next_gid[gid_];

						nex_t_state.next_real[id] += nex_t_state.next_real[id_];
						nex_t_state.next_imag[id] += nex_t_state.next_imag[id_];
					}
			}
		}

		/* get all graphs with a non zero probability */
		auto partitioned_it = __gnu_parallel::partition(nex_t_state.next_gid.begin(), nex_t_state.next_gid.begin() + total_num_graphs, [&](unsigned int const &gid) {
			/* check if graph is unique */
			if (!nex_t_state.is_first_index[gid])
				return false;

			/* check for zero probability */
			auto r = nex_t_state.next_real[gid];
			auto i = nex_t_state.next_imag[gid];

			return r*r + i*i > tolerance; 
		});

		nex_t_state.num_graphs = std::distance(nex_t_state.next_gid.begin(), partitioned_it);
				
		/* !!!!!!!!!!!!!!!!
		step (5.1) 
		 !!!!!!!!!!!!!!!! */

		if (max_num_graphs > 0 && nex_t_state.num_graphs > max_num_graphs) {

			if (verbose >= STEP_DEBUG_LEVEL)
				std::cout << "step 5.1\n";

			/* sort graphs according to probability */
			__gnu_parallel::nth_element(nex_t_state.next_gid.begin(), nex_t_state.next_gid.begin() + max_num_graphs, nex_t_state.next_gid.begin() + nex_t_state.num_graphs,
			[&](unsigned int const &gid1, unsigned int const &gid2) {
				auto r1 = nex_t_state.next_real[gid1];
				auto i1 = nex_t_state.next_imag[gid1];

				auto r2 = nex_t_state.next_real[gid2];
				auto i2 = nex_t_state.next_imag[gid2];

				return r1*r1 + i1*i1 > r2*r2 + i2*i2;
			});

			nex_t_state.num_graphs = max_num_graphs;
		}
		...
		__gnu_parallel::sort(nex_t_state.next_gid.begin(), nex_t_state.next_gid.begin() + nex_t_state.num_graphs);

		/* resize new step variables */
		nex_t_state.resize_num_graphs(nex_t_state.num_graphs);

		/* prepare for partial sum */
		for (unsigned int gid = 0; gid <  nex_t_state.num_graphs; ++gid) {
			unsigned int id = nex_t_state.next_gid[gid];

			nex_t_state.node_begin[gid + 1] = nex_t_state.node_begin[id];
			nex_t_state.sub_node_begin[gid + 1] = nex_t_state.sub_node_begin[id];

			/* assign magnitude */
			nex_t_state.real[gid] = nex_t_state.next_real[id];
			nex_t_state.imag[gid] = nex_t_state.next_imag[id];
		}

		/* compute the partial sums to get new node_begin and sub_node_begin */
		nex_t_state.node_begin[0] = 0;
		__gnu_parallel::partial_sum(nex_t_state.node_begin.begin() + 1, nex_t_state.node_begin.begin() + nex_t_state.num_graphs + 1, nex_t_state.node_begin.begin() + 1);

		nex_t_state.sub_node_begin[0] = 0;
		__gnu_parallel::partial_sum(nex_t_state.sub_node_begin.begin() + 1, nex_t_state.sub_node_begin.begin() + nex_t_state.num_graphs + 1, nex_t_state.sub_node_begin.begin() + 1);

		/* resize new step variables */
		nex_t_state.resize_num_nodes(nex_t_state.node_begin[nex_t_state.num_graphs]);
		nex_t_state.resize_num_sub_nodes(nex_t_state.sub_node_begin[nex_t_state.num_graphs]);
		...
		for (unsigned int gid = 0; gid < nex_t_state.num_graphs; ++gid) {
			...
			/* populate graphs */
			rule.populate_new_graph(*this, nex_t_state, gid, nex_t_state.parent_gid[id], nex_t_state.child_id[id]);
		}
	}
