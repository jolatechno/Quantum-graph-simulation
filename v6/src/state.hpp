...
Iteration protocol is:
  - (1): use a rule to put all operation inside of the vector "operations" and calculate the number of child graph generated by each parent graph and populate "num_childs" (*)

  - (2): compute a partial sum over "num_child" to (a) get the total number of symbolic graphs (b) know where the childs of each graphs begin and end.
  ...
  - (3, "symbolic iteartion"): by iterating over "next_gid" and using the rule, populate "next_hash", "next_real" and "next_imag" through a symbolic iteration (**),
  ...
  - (4): We first sort graphs according to "next_hash" (*****).
  ...
  - (5): compute the average memory usage of a graph.
  ...
  - (6): reserve all public vectors for the new iteration by iterating over the N first "next_gid" and using "parent_gid" and "child_id",
  	and assign "node_begin" and "sub_node_begin" by accumulating "symbolic_num_nodes" and "symbolic_num_sub_nodes".

  - (7): generate all new graphs of the new state by iterating over the N first "next_gid" and using "parent_gid" and "child_id".

  - (8): normalize the iteration, by first summing the total proba.
...
step (4) and (5) can be skiped for "fast iterations" and "probabilist iteration"
...
Load balancing:
	- steps (1) and (3) have workloads of complexity proportional to the number of nodes of previous graphs,
		they are then balanced according to section of equal total number of nodes.

	- steps (2), (4), (5), (8) have constant workloads per graph, and are balanced with sections of equal number of graphs.

	- steps (7) has a workload complexity proportional to the number of nodes of future graphs,
		they are then balanced according to section of equal total number of nodes.
*/
	...
	void step(rule_t const &rule, long long int max_num_graphs, bool overwrite_max_num_graphs, bool fast) {
		...
		#pragma omp parallel
		{
			...
			for (size_t gid = work_sharing_begin[thread_id]; gid < work_sharing_begin[thread_id + 1]; ++gid) {
				...
				current_iteration.num_childs[gid + 1] = 1 << num_ops; /* pow(2, num_ops) */
			}
