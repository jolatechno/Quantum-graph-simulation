...
Iteration protocol is:
  ...
  - (3): read the total numer of child graph generated by each parent graph and populate "next_gid", "buffer_state.symbolic_iteration.parent_gid" and "buffer_state.symbolic_iteration.child_id"
  ...
  - (7): reserve all public vector for a new state by iterating over the N first "next_gid" andusing "buffer_state.symbolic_iteration.parent_gid" and "buffer_state.symbolic_iteration.child_id",
  	and assign "node_begin" and "sub_node_begin".

  - (8): generate all new graphs of the new state by iterating over the N first "next_gid" andusing "buffer_state.symbolic_iteration.parent_gid" and "buffer_state.symbolic_iteration.child_id".
...
public:
	...
	*/
	...
	void step(state_t &buffer_state, rule_t const &rule, bool normalize) {
		...
		/* for reduction */
		size_t &total_num_graphs = buffer_state.symbolic_iteration.num_graphs;
		...
		#pragma omp parallel
		{
			...
			#pragma omp single
			{
				...
				for (unsigned int gid = 0; gid < num_graphs; ++gid) {
					...
					for (unsigned int child_id_ = 0; child_id_ < num_child; ++child_id_, ++id) {
						buffer_state.symbolic_iteration.parent_gid[id] = gid;
						buffer_state.symbolic_iteration.child_id[id] = child_id_;
					}
				}
			...
			#pragma omp single
			{
				...
				__gnu_parallel::sort(buffer_state.symbolic_iteration.next_gid.begin(), buffer_state.symbolic_iteration.next_gid.begin() + buffer_state.symbolic_iteration.num_graphs, [&](unsigned int const &gid1, unsigned int const &gid2) {
					size_t hash1 = buffer_state.symbolic_iteration.next_hash[gid1];
					size_t hash2 = buffer_state.symbolic_iteration.next_hash[gid2];

					if (hash1 != hash2)
						return hash1 > hash2;

					/* graph with less child are less costly to compute */
					return num_childs[buffer_state.symbolic_iteration.parent_gid[gid1]] < num_childs[buffer_state.symbolic_iteration.parent_gid[gid2]];
				});

				/* compute is_first_index */
				buffer_state.symbolic_iteration.is_first_index[buffer_state.symbolic_iteration.next_gid[0]] = true;
			}

			#pragma omp for
			for (unsigned int gid = 1; gid < buffer_state.symbolic_iteration.num_graphs; ++gid)
				buffer_state.symbolic_iteration.is_first_index[buffer_state.symbolic_iteration.next_gid[gid]] = buffer_state.symbolic_iteration.next_hash[buffer_state.symbolic_iteration.next_gid[gid]] != buffer_state.symbolic_iteration.next_hash[buffer_state.symbolic_iteration.next_gid[gid - 1]];
			...
			#pragma omp for
			for (unsigned int gid = 0; gid < buffer_state.symbolic_iteration.num_graphs; ++gid) {
				auto id = buffer_state.symbolic_iteration.next_gid[gid];

				if (buffer_state.symbolic_iteration.is_first_index[id])
					/* sum magnitude of equal graphs */
					for (unsigned int gid_ = gid + 1; gid_ < buffer_state.symbolic_iteration.num_graphs && !buffer_state.symbolic_iteration.is_first_index[buffer_state.symbolic_iteration.next_gid[gid_]]; ++gid_) {
						auto id_ = buffer_state.symbolic_iteration.next_gid[gid_];

						buffer_state.symbolic_iteration.next_real[id] += buffer_state.symbolic_iteration.next_real[id_];
						buffer_state.symbolic_iteration.next_imag[id] += buffer_state.symbolic_iteration.next_imag[id_];
					}
			}
			...
			#pragma omp single
			{
				...
				auto partitioned_it = __gnu_parallel::partition(buffer_state.symbolic_iteration.next_gid.begin(), buffer_state.symbolic_iteration.next_gid.begin() + buffer_state.symbolic_iteration.num_graphs, [&](unsigned int const &gid) {
					/* check if graph is unique */
					if (!buffer_state.symbolic_iteration.is_first_index[gid])
						return false;

					/* check for zero probability */
					PROBA_TYPE r = buffer_state.symbolic_iteration.next_real[gid];
					PROBA_TYPE i = buffer_state.symbolic_iteration.next_imag[gid];

					return r*r + i*i > tolerance; 
				});

				buffer_state.num_graphs = std::distance(buffer_state.symbolic_iteration.next_gid.begin(), partitioned_it);
				...
				if (buffer_state.num_graphs > max_num_graphs) {
					...
					__gnu_parallel::nth_element(buffer_state.symbolic_iteration.next_gid.begin(), buffer_state.symbolic_iteration.next_gid.begin() + max_num_graphs, buffer_state.symbolic_iteration.next_gid.begin() + buffer_state.num_graphs,
					[&](unsigned int const &gid1, unsigned int const &gid2) {
						PROBA_TYPE r1 = buffer_state.symbolic_iteration.next_real[gid1];
						PROBA_TYPE i1 = buffer_state.symbolic_iteration.next_imag[gid1];

						PROBA_TYPE r2 = buffer_state.symbolic_iteration.next_real[gid2];
						PROBA_TYPE i2 = buffer_state.symbolic_iteration.next_imag[gid2];
				...
				__gnu_parallel::sort(buffer_state.symbolic_iteration.next_gid.begin(), buffer_state.symbolic_iteration.next_gid.begin() + buffer_state.num_graphs);
			...
			for (unsigned int gid = gid_seq + 1; gid < buffer_state.num_graphs; ++gid) {
				...
				buffer_state.imag[gid] = buffer_state.symbolic_iteration.next_imag[id];
			}
			...
			for (unsigned int gid = 0; gid < buffer_state.num_graphs; ++gid) {
				auto id = buffer_state.symbolic_iteration.next_gid[gid];
				/* populate graphs */
				rule.populate_new_graph(*this, buffer_state, gid, buffer_state.symbolic_iteration.parent_gid[id], buffer_state.symbolic_iteration.child_id[id]);
			}
		}
