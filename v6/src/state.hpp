...
Iteration protocol is:
	- (1): normalize the old iteration here if wanted, by first summing the total proba.
  	and then go over all graphs and divide their magnitude by the square root of the total probability.

  - (2): use a rule to put all operation inside of the vector "operations".

  - (3): calculate the number of child graph generated by each parent graph and populate "num_childs" (*)

  - (4): compute a partial sum over "num_child" to (a) get the total number of symbolic graphs (b) know where the childs of each graphs begin and end.
  ...
  - (5, "symbolic iteartion"): by iterating over "next_gid" and using the rule, populate "next_hash", "next_real" and "next_imag" through a symbolic iteration (**),
  ...
  - (6), (f), (p): using "next_hash" sum the probability ("next_real" and "next_imag") of equal graphs.
	...
	- (7), (p): compute the average memory usage of a graph.
  ...
  - (8): reserve all public vectors for the new iteration by iterating over the N first "next_gid" and using "parent_gid" and "child_id",
  	and assign "node_begin" and "sub_node_begin" by accumulating "symbolic_num_nodes" and "symbolic_num_sub_nodes".

  - (9): generate all new graphs of the new state by iterating over the N first "next_gid" and using "parent_gid" and "child_id".
	...
	*/
	...
	typedef struct iteration {
		...
		*/
		...
		numa_vector<unsigned int> num_childs; /* of size (a) + 1 */
...
*/
	...
	long int inline get_max_num_graphs() const {
		...
		static const long int graph_mem_usage = 2*sizeof(PROBA_TYPE) + 3*sizeof(unsigned int);
