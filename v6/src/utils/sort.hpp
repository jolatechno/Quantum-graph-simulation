...
void inline parallel_radix_count_indexed_0(unsigned long int *begin, unsigned long int *end,
	size_t *valueBegin,
	...
	for (unsigned long int *it = begin; it != end; ++it) {
...
void inline radix_secon_loop_indexed_offset(unsigned long int *begin, unsigned long int *outBegin,
	size_t size,
	...
	for (int i = size - 1; i >= 0; --i) {
		unsigned long int idx = --count[char_valueBegin[8*begin[i] + offset]];
		outBegin[idx] = begin[i];
	}
}

// sequential contracted radix sort for byte 7 to 1 for indexed values
void inline radix_indexed_sort_1_7(unsigned long int *begin, unsigned long int *end,
	unsigned long int *outBegin,
	size_t *valueBegin) {

	// declaration of the count variable
	unsigned long int count1[256] = {0};
	unsigned long int count2[256] = {0};
	unsigned long int count3[256] = {0};
	unsigned long int count4[256] = {0};
	unsigned long int count5[256] = {0};
	unsigned long int count6[256] = {0};
	unsigned long int count7[256] = {0};
