...
def make_quantum_cmd(args, theta):
	return f"../../quantum_iterations.out --start-serializing { max(0, args.n_iter - args.n_serializing + 1) } -s { args.size } -n { args.n_iter } --theta { theta } --phi 0 --seed 0 " + " ".join(args.args + args.q_args)
